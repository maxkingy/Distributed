...
public class QueueHandler {
	...
	/*
	 * Method to add a request to the queue. Input argument is the
	 * main.weatherService.ClientRequest object. Queue has a capacity of 100
	 * requests to limit memory and resource usage (This could be altered depending
	 * on the requirements of the system). Client request is added to the queue
	 * depending on it's lamport timestamp primarily and then its PID.
	 */
	public void addRequestToQueue(ClientRequest req) {
		try {
			...
			for (ClientRequest cr : queue) {
				...
				if (lamportCompare < 0 || (lamportCompare == 0
						&& Integer.parseInt(cr.getPID()) < Integer.parseInt(currReq.getPID()))) {
					/*
					 * Add the existing request to the new queue if the timestamp is lower, or if
					 * equal and PID is lower
					 */
					newQueue.add(cr);
				} else {
	...
	/*
	 * Method to retrieve and remove the next request from the queue. Method returns
	 * the next client request to handle.
	 */
