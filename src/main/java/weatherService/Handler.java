...
public class Handler implements Runnable {
	...
	/*
	 * Simple constructor used by aggregation server. Input arguments are the
	 * main.weatherService.ClientRequest object for details about the request and a
	...
	/*
	 * Override run method from Runnable. Method will attempt to acquire the
	 * semaphore and then call the helper method to process the client request.
	 * Semaphore must be released afterward.
	 */
	@Override
	public void run() {
		try {
	...
	/*
	 * Helper method to process a request made by a client. Input argument is the
	 * main.weatherService.ClientRequest object. Method either calls the PUT or GET
	 * handler if the input specifies, otherwise a 400 response is returned.
	 */
	...
	/*
	 * Helper method to process a PUT request. Input arguments are the
	 * main.weatherService.ClientRequest object and the socket. Method gets the
	 * saved json data (saved by the aggregation server) and uses it to form the
	 * correct response.
	 */
	private void processPUTRequest(ClientRequest req, Socket socket) {
		...
		if (file.exists()) {
			...
			if (file.length() == 0) {
				...
				if (obj == null) {
					response = new Response("Internal Server Error", "500", "0", "", "Invalid JSON data", socket);
					response.respond();
				} else {
					upload(obj);
					if (!Database.getInstance().alreadyConnected(req.getContentServerID())) {
						// If the content server is not already connected, add to
						// connectedContentServers
	...
	/*
	 * Helper method to check if saved json data is valid. This method is called by
	 * "processPUTRequest". Input argument is the name of the file containing the
	 * saved json data. Method returns a JSONObject containing the data if it is
	 * valid, otherwise will return null.
	 */
			...
			for (int i = 0; i < jArray.length(); i++) {
				try {
					...
					if (!json.has("id") || !json.has("name") || !json.has("state") || !json.has("time_zone")
							...
							|| !json.has("cloud") || !json.has("dewpt") || !json.has("press") || !json.has("rel_hum")
							|| !json.has("wind_dir") || !json.has("wind_spd_kmh") || !json.has("wind_spd_kt")) {
	...
	/*
	 * Helper method to upload new data to the database. Input argument is the
	 * JSONObject containing the weather data.
	 */
	...
	/*
	 * Helper method to process a GET request. Input arguments are the
	 * main.weatherService.ClientRequest object and the socket. The response created
	 * by the method is dependent on whether there is data for the specified content
