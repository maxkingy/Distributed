...
public class GETClient {
  ...
  /*
   Helper method to build header for response. Inputs arguments are the host, the station ID, the
   lamport timestamp and the process ID. Returns a String of the header.
   */
  ...
  /*
   Helper method to parse and display weather data. Input argument is the weather data in String
   form. This method uses a string builder, JSONObjects and a JSONArray to put together the weather
   data output to the GET client.
   */
  private void parseAndDisplayWeatherData(String data) {
    try {
      JSONObject obj = new JSONObject(data);
      StringBuilder weatherData =
      ...
      for (int i = 0; i < array.length(); i++) {
        ...
        try {
          ...
          weatherData.append("Local Date Time: ").append(json.getString("local_date_time"))
              .append("\n");
          weatherData.append("Local Date Time Full: ")
              .append(json.getString("local_date_time_full")).append("\n");
          weatherData.append("Air Temperature: ").append(json.getDouble("air_temp")).append("\n");
          weatherData.append("Apparent Temperature: ").append(json.getDouble("apparent_t"))
              .append("\n");
          ...
          weatherData.append("Wind Speed Kmh: ").append(json.getDouble("wind_spd_kmh"))
              .append("\n");
  ...
  /*
   Main method of the main.weatherService.GETClient class. Command-line input arguments are the server URI and
   optionally, a second input argument which tells the aggregation server what weather data to
   retrieve via station ID. If the second argument is not passed to main, the aggregation server
   will supply the most recent data.

   Method sends GET requests to the aggregation server via the socket. The response received by the
   client is printed to the user in both this method and by calling the helper method
  ...
  public static void main(String[] args) {
    ...
    try (Socket socket = new Socket(serverName, serverPort);
      ...
      clock.tick();
      String header = client.buildHeader(serverName, stationID,
